...
{
    CfalcoinAddress addressParsed(address.toStdString());
...
WalletModel::SendCoinsReturn WalletModel::sendCoins(const QList<SendCoinsRecipient> &recipients, const CCoinControl *coinControl)
{
    ...
    foreach(const SendCoinsRecipient &rcp, recipients)
    {
        ...
        else if (rcp.IsChat())
        {
            ...
            chat_message = rcp.message.toStdString();
            chat_data = rcp.data.toStdString();
        }
    }
...

    {
        ...
        foreach(const SendCoinsRecipient &rcp, recipients)
        {
            ...
            scriptPubKey.SetDestination(CfalcoinAddress(rcp.address.toStdString()).Get());
        ...
        if(bAskFee && !uiInterface.ThreadSafeAskFee(nFeeRequired))
        {
            return Aborted;
        }
...
{
    OutputDebugStringF("NotifyAddressBookChanged %s %s isMine=%i status=%i\n", CfalcoinAddress(address).ToString().c_str(), label.c_str(), isMine, status);
    QMetaObject::invokeMethod(walletmodel, "updateAddressBook", Qt::QueuedConnection,
                              Q_ARG(QString, QString::fromStdString(CfalcoinAddress(address).ToString())),
...
void WalletModel::listCoins(std::map<QString, std::vector<COutput> >& mapCoins) const
{
    ...
    BOOST_FOREACH(const COutput& out, vCoins)
    {
        ...
        mapCoins[CfalcoinAddress(address).ToString().c_str()].push_back(out);
    }
}
