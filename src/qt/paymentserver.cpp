...
const QString TALKCOIN_IPC_PREFIX("falcoin:");
...
bool PaymentServer::ipcSendCommandLine()
{
    ...
    {
        if (!args[i].startsWith(TALKCOIN_IPC_PREFIX, Qt::CaseInsensitive))
            continue;
...
{
    // Install global event filter to catch QFileOpenEvents on the mac (sent when you click falcoin: links)
    ...
    if (!uriServer->listen(name))
        qDebug() << tr("Cannot start falcoin: click-to-pay handler");
    else
...
{
    // clicking on falcoin: URLs creates FileOpen events on the Mac:
