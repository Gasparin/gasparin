...
{
    QString ret = QString("falcoin:%1").arg(address);
    int paramCount = 0;
    ...
    if (ui->chkReqPayment->isChecked())
    {
        ...
        {
            // even if we allow a non FAL unit input in lnReqAmount, we generate the URI with FAL as unit (as defined in BIP21)
            ret += QString("?amount=%1").arg(falcoinUnits::format(falcoinUnits::FAL, ui->lnReqAmount->value()));
            paramCount++;
        }
        else
        {
