...
// Copyright (c) 2009-2012 falcoin Developers
...
const string strMessageMagic = "falcoin Signed Message:\n";
...
{
    // falcoin: IsDust() detection disabled, allows any valid dust to be relayed.
        ...
        BOOST_FOREACH(const CTransaction& tx, pblock->vtx)
        {
            ...
            if (tx.nVote >= GET_V_REWARDMIN(nHeight) && tx.nVote <= GET_V_REWARDMAX() && tx.vout.size() == 2)
            {
                for (unsigned int i = 0; i < tx.vout.size(); i++)
                {
                    ...
                    {
                        if (CfalcoinAddress(address).ToBase64() == GET_A_VOTE1(nHeight) && txout.nValue == GET_V_VOTE(nHeight))
                        {
                            V += tx.nVote;
                            V_Total++;
                            break;
            ...
            else if (tx.nVote2 >= 0 && tx.nVote2 <= GET_V_REWARDMAX() && tx.vout.size() == 2)
            {
                for (unsigned int i = 0; i < tx.vout.size(); i++)
                {
                    ...
                    if (ExtractDestination(txout.scriptPubKey, address))
                    {
                        if (CfalcoinAddress(address).ToBase64() == GET_A_VOTE2(nHeight) && txout.nValue == GET_V_VOTE(nHeight))
                        {
                            V2 += tx.nVote2;
                            V2_Total++;
                            break;
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // falcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::ConnectBlock(CValidationState &state, CBlockIndex* pindex, CCoinsViewCache &view, bool fJustCheck)
{
    ...
    CTxDestination address;
    if (!(ExtractDestination(txout.scriptPubKey, address) && CfalcoinAddress(address).ToBase64() == GET_A_SHARE() && txout.nValue == _V2))
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // falcoin: Special short-term limits to avoid 10,000 BDB lock limit:
        ...
        if (tx.TLKtime > 0 && !tx.TLKnick.empty() && !tx.TLKmsg.empty() && pwalletMain->checkVersion(tx.TLKdata) && tx.vout.size() == 2)
        {
            ...
            for (unsigned int i = 0; i < tx.vout.size(); i++)
            {
                ...
                if (ExtractDestination(txout.scriptPubKey, address))
                {
                    ...
                    if (CfalcoinAddress(address).ToBase64() == GET_A_CHAT(nHeight) && (txout.nValue == GET_V_CHAT(nHeight) || txout.nValue == GET_V_CHATB(nHeight)))
                    {
                        bTX = true;
                        nValue = txout.nValue;
...
{
    // falcoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        block.nVersion = 1;
        block.nTime    = 1404867575;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 1067381;
...
unsigned char pchMessageStart[4] = { 0x32, 0x1a, 0x9f, 0xa3 }; // falcoin: increase each by adding 2 to falcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("falcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("falcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static falcoinMiner(CWallet *pwallet)
{
    printf("falcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("falcoin-miner");
    ...
    try { loop {
        ...
        printf("Running falcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("falcoinMiner terminated\n");
        throw;
    }
}

void Generatefalcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&falcoinMiner, pwallet));
}
