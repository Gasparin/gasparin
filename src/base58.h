...
/** base58-encoded falcoin addresses.
 * Public-key-hash-addresses have version 0 (or 111 testnet).
 * The data vector contains RIPEMD160(SHA256(pubkey)), where pubkey is the serialized public key.
 * Script-hash-addresses have version 5 (or 196 testnet).
 * The data vector contains RIPEMD160(SHA256(cscript)), where cscript is the serialized redemption script.
 */
class CfalcoinAddress;
class CfalcoinAddressVisitor : public boost::static_visitor<bool>
{
private:
    CfalcoinAddress *addr;
public:
    CfalcoinAddressVisitor(CfalcoinAddress *addrIn) : addr(addrIn) { }
    bool operator()(const CKeyID &id) const;
    bool operator()(const CScriptID &id) const;
    bool operator()(const CNoDestination &no) const;
};

class CfalcoinAddress : public CBase58Data
{
public:
    enum
    {
    ...
    {
        return boost::apply_visitor(CfalcoinAddressVisitor(this), dest);
    }
    ...
    CfalcoinAddress(const CTxDestination &dest)
    {
        Set(dest);
    }

    CfalcoinAddress(const std::string& strAddress)
    {
        SetString(strAddress);
    }

    CfalcoinAddress(const char* pszAddress)
    {
...
{
    CfalcoinAddress addr(DecodeBase64(x));
    return addr.GetHash160();
}

bool inline CfalcoinAddressVisitor::operator()(const CKeyID &id) const         { return addr->Set(id); }
bool inline CfalcoinAddressVisitor::operator()(const CScriptID &id) const      { return addr->Set(id); }
bool inline CfalcoinAddressVisitor::operator()(const CNoDestination &id) const { return false; }

/** A base58-encoded secret key */
class CfalcoinSecret : public CBase58Data
{
public:
    enum
    {
        PRIVKEY_ADDRESS = CfalcoinAddress::PUBKEY_ADDRESS + 128,
        PRIVKEY_ADDRESS_TEST = CfalcoinAddress::PUBKEY_ADDRESS_TEST + 128,
    };
    ...
    CfalcoinSecret(const CKey& vchSecret)
    {
        SetKey(vchSecret);
    }
